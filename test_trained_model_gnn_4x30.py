#!/usr/bin/env python
"""
Test/Evaluation Script for Trained GNN-HAPPO Models — 4 DC × 30 Retailer Scale

Wraps GNNModelEvaluator from test_trained_model_gnn_2x15.py, overriding environment
creation to use the 4x30 config.

Usage:
    python test_trained_model_gnn_4x30.py \
        --model_dir results/gnn_happo_4x30/run_seed_1/models \
...
class GNNModelEvaluator4x30(GNNModelEvaluator):
    """Evaluator subclass for the 4 DC × 30 Retailer scaled environment."""

    RESULTS_CSV_NAME = 'results_gnn_happo_4x30.csv'

    def _create_env(self):
        print('Creating evaluation environment (4×30 scale)...')
        parser = get_config()
        parser.set_defaults(
            env_name='MultiDC',
    ...
    def _build_graph(self):
        print('Building supply chain graph (4×30)...')
    ...
    def _print_header(self):
        print('=' * 70)
        print('GNN-HAPPO Model Evaluation — 4×30 Scale')
        print('=' * 70)
    ...
    def _print_summary(self, stats):
        print('\n' + '=' * 70)
        print('GNN-HAPPO Evaluation Summary — 4×30 Scale')
        print('=' * 70)
...
def main():
    args = parse_args()
    evaluator = GNNModelEvaluator4x30(args)
    evaluator.evaluate()
