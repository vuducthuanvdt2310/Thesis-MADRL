#!/usr/bin/env python
"""
Test/Evaluation Script for Trained HAPPO Models -- 1 DC x 3 Retailer Scale

Subclasses HAPPOEvaluator2x30 to reuse its topology-aware episode runner,
plotter, and statistics logic. Only overrides env creation, CSV naming,
and summary printing.

Usage:
    python test_trained_model_happo_1x3.py \
        --model_dir results/happo_1x3/run_seed_1/models \
...
N_DCS = 1
N_RETAILERS = 3
N_AGENTS = N_DCS + N_RETAILERS  # 4
CONFIG_PATH = 'configs/multi_dc_1x3_config.yaml'


class HAPPOEvaluator1x3(HAPPOEvaluator2x30):
    """HAPPO evaluator for the 1 DC x 3 Retailer environment."""

    def __init__(self, args):
        self.n_dcs = N_DCS
        self.policy_class_name = 'happo'
        # Skip HAPPOEvaluator2x30.__init__ to avoid setting n_dcs=2;
        # go straight to ModelEvaluator via the grandparent class.
        from test_trained_model_happo_2x15 import ModelEvaluator
        ModelEvaluator.__init__(self, args)

    def _create_env(self):
        print('Creating evaluation environment (1x3 scale)...')
        parser = get_config()
        parser.set_defaults(
            env_name='MultiDC',
            scenario_name='inventory_2echelon_1x3',
            num_agents=N_AGENTS,
    ...
    def _save_metrics_csv(self):
        ...
        results_path = self.save_dir / 'results_standard_happo_1x3.csv'
    ...
    def _print_summary(self, stats):
        print('\n' + '=' * 70)
        print('HAPPO Evaluation Summary -- 1x3 Scale')
        print('=' * 70)
...
def main():
    ...
    evaluator = HAPPOEvaluator1x3(args)
    evaluator.evaluate()
