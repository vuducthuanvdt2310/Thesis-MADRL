#!/usr/bin/env python
"""
Test/Evaluation Script for Trained HAPPO Models — 4 DC × 40 Retailer Scale
===========================================================================

Fully self-contained evaluator that inherits directly from
``test_trained_model.ModelEvaluator`` and inlines every topology-aware
override so that **no dependency on ``test_evaluator_topology``** exists.

Usage:
    python test_trained_model_happo_4x40.py \
        --model_dir results/happo_4x40/run_seed_1/models \
...
class HAPPOEvaluator4x40(ModelEvaluator):
    """HAPPO evaluator for the 4 DC × 40 Retailer environment.

    Inherits directly from ``ModelEvaluator`` and overrides all methods that
    need topology awareness (DC count, policy class, etc.) so that the file
    ...
    def _save_metrics_csv(self):
        ...
        results_path = self.save_dir / 'results_standard_happo_4x40.csv'
    ...
    def _print_summary(self, stats):
        print('\n' + '=' * 70)
        print(f'HAPPO Evaluation Summary \u2014 4\u00d740 Scale')
        print('=' * 70)
...
def main():
    ...
    evaluator = HAPPOEvaluator4x40(args)
    evaluator.evaluate()
