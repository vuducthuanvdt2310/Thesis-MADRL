#!/usr/bin/env python
"""
Test/Evaluation Script for Trained GNN-HAPPO Models — 2 DC × 40 Retailer Scale

Wraps GNNModelEvaluator from test_trained_model_gnn_2x15.py, overriding environment
creation to use the 2x40 config.

Usage:
    python test_trained_model_gnn_2x40.py \
        --model_dir results/gnn_happo_2x40/run_seed_1/models \
...
class GNNModelEvaluator2x40(GNNModelEvaluator):
    """Evaluator subclass for the 2 DC × 40 Retailer scaled environment."""

    RESULTS_CSV_NAME = 'results_gnn_happo_2x40.csv'

    def _create_env(self):
        print('Creating evaluation environment (2×40 scale)...')
        parser = get_config()
        parser.set_defaults(
            env_name='MultiDC',
    ...
    def _build_graph(self):
        print('Building supply chain graph (2×40)...')
    ...
    def _print_header(self):
        print('=' * 70)
        print('GNN-HAPPO Model Evaluation — 2×40 Scale')
        print('=' * 70)
    ...
    def _print_summary(self, stats):
        print('\n' + '=' * 70)
        print('GNN-HAPPO Evaluation Summary — 2×40 Scale')
        print('=' * 70)
...
def main():
    args = parse_args()
    evaluator = GNNModelEvaluator2x40(args)
    evaluator.evaluate()
