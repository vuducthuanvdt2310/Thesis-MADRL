#!/usr/bin/env python
"""
Test/Evaluation Script for Trained MAPPO Models -- 1 DC x 10 Retailer Scale

Subclasses MAPPOEvaluator2x30 to reuse its topology-aware episode runner.

Usage:
    python test_trained_model_mappo_1x10.py \
        --model_dir results/mappo_1x10/run_seed_1/models \
...
N_DCS = 1
N_RETAILERS = 10
N_AGENTS = N_DCS + N_RETAILERS  # 11
CONFIG_PATH = 'configs/multi_dc_1x10_config.yaml'


class MAPPOEvaluator1x10(MAPPOEvaluator2x30):
    """MAPPO evaluator for the 1 DC x 10 Retailer environment."""
    ...
    def _create_env(self):
        print('Creating evaluation environment (1x10 scale)...')
        parser = get_config()
        parser.set_defaults(
            env_name='MultiDC',
            scenario_name='inventory_2echelon_1x10',
            num_agents=N_AGENTS,
    ...
    def _save_metrics_csv(self):
        ...
        results_path = self.save_dir / 'results_standard_mappo_1x10.csv'
    ...
    def _print_summary(self, stats):
        print('\n' + '=' * 70)
        print('MAPPO Evaluation Summary -- 1x10 Scale')
        print('=' * 70)
...
def main():
    ...
    evaluator = MAPPOEvaluator1x10(args)
    evaluator.evaluate()
