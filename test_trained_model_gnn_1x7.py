#!/usr/bin/env python
"""
Test/Evaluation Script for Trained GNN-HAPPO Models — 1 DC × 7 Retailer Scale

Wraps the existing GNNModelEvaluator from test_trained_model_gnn.py,
overriding environment creation to use the 1x7 config.

Usage:
    python test_trained_model_gnn_1x7.py \
        --model_dir results/gnn_happo_1x7/run_seed_1/models \
...
class GNNModelEvaluator1x7(GNNModelEvaluator):
    """Evaluator subclass for the 1 DC × 7 Retailer environment."""

    RESULTS_CSV_NAME = 'results_gnn_happo_1x7.csv'

    def _create_env(self):
        """Override: create env with 1x7 config."""
        print('Creating evaluation environment (1×7 scale)...')
        parser = get_config()
        parser.set_defaults(
            env_name='MultiDC',
        ...
        print(f'     Agents         : {self.n_agents} ({N_DCS} DC + {N_RETAILERS} Retailers)')
    ...
    def _build_graph(self):
        """Override: build graph for 1 DC + 7 retailers."""
        print('Building supply chain graph (1×7)...')
    ...
    def _run_episode(self, episode_num, save_trajectory=False):
        """Override: use N_DCS for DC detection."""
        metrics = super()._run_episode(episode_num, save_trajectory)
        return metrics

    def _print_header(self):
        print('=' * 70)
        print('GNN-HAPPO Model Evaluation — 1×7 Scale')
        print('=' * 70)
        print(f'Model directory : {self.args.model_dir}')
        print(f'Config          : {CONFIG_PATH}')
        print(f'Topology        : 1 Supplier → {N_DCS} DC → {N_RETAILERS} Retailers')
    ...
    def _print_summary(self, stats):
        print('\n' + '=' * 70)
        print('GNN-HAPPO Evaluation Summary — 1×7 Scale')
        print('=' * 70)
...
def main():
    args = parse_args()
    evaluator = GNNModelEvaluator1x7(args)
    evaluator.evaluate()
