#!/usr/bin/env python
"""
Test/Evaluation Script for Trained MAPPO Models -- 2 DC x 40 Retailer Scale

Subclasses MAPPOEvaluator2x30 to reuse its topology-aware episode runner.

Usage:
    python test_trained_model_mappo_2x40.py \
        --model_dir results/mappo_2x40/run_seed_1/models \
...
N_DCS = 2
N_RETAILERS = 40
N_AGENTS = N_DCS + N_RETAILERS  # 42
CONFIG_PATH = 'configs/multi_dc_2x40_config.yaml'


class MAPPOEvaluator2x40(MAPPOEvaluator2x30):
    """MAPPO evaluator for the 2 DC x 40 Retailer environment."""
    ...
    def _create_env(self):
        print('Creating evaluation environment (2x40 scale)...')
        parser = get_config()
        parser.set_defaults(
            env_name='MultiDC',
            scenario_name='inventory_2echelon_2x40',
            num_agents=N_AGENTS,
    ...
    def _save_metrics_csv(self):
        ...
        results_path = self.save_dir / 'results_standard_mappo_2x40.csv'
    ...
    def _print_summary(self, stats):
        print('\n' + '=' * 70)
        print('MAPPO Evaluation Summary -- 2x40 Scale')
        print('=' * 70)
...
def main():
    ...
    evaluator = MAPPOEvaluator2x40(args)
    evaluator.evaluate()
