#!/usr/bin/env python
"""
Topology-Aware Evaluator for Multi-DC Inventory Management
============================================================

Shared base module that provides topology-aware evaluation by subclassing
``ModelEvaluator`` from ``test_trained_model.py``.  All methods that formerly
hardcoded the DC count as ``2`` are overridden here so that the number of DCs
is driven by ``self.n_dcs`` (set by concrete subclasses in ``_create_env``).

The module also generalises policy loading so that both HAPPO and MAPPO
trained weights can be evaluated through the ``self.policy_class_name``
attribute (``'happo'`` or ``'mappo'``).

Usage:
    Subclass ``TopologyAwareEvaluator``, override ``_create_env`` to set
    ``self.n_dcs``, ``self.policy_class_name``, and create/return the
...
class TopologyAwareEvaluator(ModelEvaluator):
    """Evaluator that replaces every hardcoded DC-count (``2``) with ``self.n_dcs``.

    Attributes:
        n_dcs (int): Number of distribution centres.  Defaults to ``2`` so that
            the base class works for the standard 2-DC / 15-retailer topology
            out of the box.  Subclasses should set this in ``_create_env``.
        policy_class_name (str): ``'happo'`` or ``'mappo'``.  Controls which
            ``Policy`` class is imported in ``_load_models``.  Defaults to
            ``'happo'``.
    """
    ...
    def _load_models(self):
        ...
        Identical to the parent except that the policy class is chosen based on
        ``self.policy_class_name`` (``'happo'`` or ``'mappo'``) instead of
        always importing ``HAPPO_Policy``.
        """
    ...
    def _run_episode(self, episode_num, save_trajectory=False):
        ...
        This is a full copy of ``ModelEvaluator._run_episode`` with every
