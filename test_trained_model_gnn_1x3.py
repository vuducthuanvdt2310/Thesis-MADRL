#!/usr/bin/env python
"""
Test/Evaluation Script for Trained GNN-HAPPO Models — 1 DC × 3 Retailer Scale

Wraps GNNModelEvaluator from test_trained_model_gnn_2x15.py, overriding environment
creation to use the 1x3 config.

Usage:
    python test_trained_model_gnn_1x3.py \
        --model_dir results/gnn_happo_1x3/run_seed_1/models \
...
class GNNModelEvaluator1x3(GNNModelEvaluator):
    """Evaluator subclass for the 1 DC × 3 Retailer scaled environment."""

    RESULTS_CSV_NAME = 'results_gnn_happo_1x3.csv'

    def _create_env(self):
        print('Creating evaluation environment (1×3 scale)...')
        parser = get_config()
        parser.set_defaults(
            env_name='MultiDC',
    ...
    def _build_graph(self):
        print('Building supply chain graph (1×3)...')
    ...
    def _print_header(self):
        print('=' * 70)
        print('GNN-HAPPO Model Evaluation — 1×3 Scale')
        print('=' * 70)
    ...
    def _print_summary(self, stats):
        print('\n' + '=' * 70)
        print('GNN-HAPPO Evaluation Summary — 1×3 Scale')
        print('=' * 70)
...
def main():
    args = parse_args()
    evaluator = GNNModelEvaluator1x3(args)
    evaluator.evaluate()
