#!/usr/bin/env python
"""
Test/Evaluation Script for Trained HAPPO Models -- 1 DC x 7 Retailer Scale
(Fully self-contained: inherits directly from ModelEvaluator with all
topology-aware overrides inlined.)

Usage:
    python test_trained_model_happo_1x7.py \
        --model_dir results/happo_1x7/run_seed_1/models \
...
N_DCS = 1
N_RETAILERS = 7
N_AGENTS = N_DCS + N_RETAILERS  # 8
CONFIG_PATH = 'configs/multi_dc_1x7_config.yaml'


class HAPPOEvaluator1x7(ModelEvaluator):
    """HAPPO evaluator for the 1 DC x 7 Retailer environment.

    Fully self-contained: all topology-aware overrides are inlined from
    TopologyAwareEvaluator so that this file has no dependency on
    test_evaluator_topology.py.
    """
    ...
    def _create_env(self):
        print('Creating evaluation environment (1x7 scale)...')
        parser = get_config()
        parser.set_defaults(
            env_name='MultiDC',
            scenario_name='inventory_2echelon_1x7',
            num_agents=N_AGENTS,
        ...
        print(f'[OK] Environment: {self.args.num_agents} agents '
              f'({N_DCS} DC + {N_RETAILERS} Retailers)')
    ...
    def _save_metrics_csv(self):
        ...
        results_path = self.save_dir / 'results_standard_happo_1x7.csv'
    ...
    def _print_summary(self, stats):
        print('\n' + '=' * 70)
        print(f'HAPPO Evaluation Summary -- 1x7 Scale')
        print('=' * 70)
...
def main():
    ...
    evaluator = HAPPOEvaluator1x7(args)
    evaluator.evaluate()
