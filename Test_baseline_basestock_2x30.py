#!/usr/bin/env python
"""
Baseline Evaluation Script: (s,S) Heuristic Policy -- 2 DC x 30 Retailer Scale
==============================================================================

Thin wrapper around `Test_baseline_basestock.py` that points the environment
to the 2x30 topology config. All evaluation logic, metrics, plots, CSV/JSON
...
Usage (all arguments optional):
    python Test_baseline_basestock_2x30.py
    python Test_baseline_basestock_2x30.py --num_episodes 5 --episode_length 365
    python Test_baseline_basestock_2x30.py \\
        --s_dc 100 --S_dc 170 --s_retailer 3 --S_retailer 10 \\
        --experiment_name "basestock_2x30"
"""
...
class BaseStockEvaluator2x30(BaseStockEvaluator):
    """2x30 topology-specific evaluator. Only overrides the output CSV name."""
    RESULTS_CSV_NAME = 'results_ss_heuristic_2x30.csv'
...
def main():
    ...
    print('\n=== (s,S) Heuristic Configuration (2x30) ===')
    ...
    evaluator = BaseStockEvaluator2x30(args)
    evaluator.evaluate()
