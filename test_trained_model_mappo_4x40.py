#!/usr/bin/env python
"""
Test/Evaluation Script for Trained MAPPO Models — 4 DC x 40 Retailer Scale
===========================================================================
...
override so that no import from ``test_evaluator_topology`` is needed.

Usage:
    python test_trained_model_mappo_4x40.py \
        --model_dir results/mappo_4x40/run_seed_1/models \
...
class MAPPOEvaluator4x40(ModelEvaluator):
    """MAPPO evaluator for the 4 DC x 40 Retailer environment.

    Inherits directly from ``ModelEvaluator`` and inlines all
    topology-aware overrides so that the file is fully self-contained.
    """
    ...
    def _create_env(self):
        print('Creating evaluation environment (4x40 scale)...')
        parser = get_config()
        parser.set_defaults(
            env_name='MultiDC',
    ...
    def _load_models(self):
        ...
        Uses ``self.policy_class_name`` to select the correct policy class
        (``'mappo'`` -> ``MAPPO_Policy``).
        """
    ...
    def _run_episode(self, episode_num, save_trajectory=False):
        ...
        Full topology-aware override with every hardcoded DC-count replaced
        by ``self.n_dcs``.
        """
    ...
    def _save_metrics_csv(self):
        ...
        results_path = self.save_dir / 'results_standard_mappo_4x40.csv'
    ...
    def _print_summary(self, stats):
        print('\n' + '=' * 70)
        print('MAPPO Evaluation Summary \u2014 4\u00d740 Scale')
        print('=' * 70)
...
def main():
    ...
    evaluator = MAPPOEvaluator4x40(args)
    evaluator.evaluate()
