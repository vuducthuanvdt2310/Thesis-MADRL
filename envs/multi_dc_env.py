...
class MultiDCInventoryEnv:
    ...
    def _define_spaces(self):
        ...
        # Retailer Observation: 42D (14 features × 3 SKUs)
        self.obs_dim_retailer = self.n_skus * 14
    ...
    def _get_demand(self, retailer_idx: int, day: int) -> np.ndarray:
        ...
        """
        # Simple synthetic demand (can be replaced with CSV data)
        base_demand = np.array([15, 10, 20], dtype=np.float32)
        
        # Add some variability
        noise = np.random.normal(0, 2, self.n_skus)
        ...
        demand = demand * retailer_multipliers[retailer_idx]
    ...
    def _get_retailer_observation(self, retailer_id: int) -> np.ndarray:
        """
        Build observation for a retailer agent (42 dimensions).
        
        Features per SKU (14 total):
        1. Own inventory
        2. Own backlog
