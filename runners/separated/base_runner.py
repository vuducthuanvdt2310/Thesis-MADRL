...
class BaseRunner(object):
    def __init__(self, config):
        ...
        if self.all_args.algorithm_name == "happo":
            ...
            print("\n" + "="*70)
            print("Building supply chain graph for GNN-HAPPO...")
            print("="*70)
            _n_dcs = int(getattr(self.all_args, 'n_dcs', 2))
            adj = build_supply_chain_adjacency(n_dcs=_n_dcs, n_retailers=self.num_agents - _n_dcs, self_loops=True)
        ...
        for agent_id in range(self.num_agents):
            ...
            if self.all_args.algorithm_name == "gnn_happo":
                # GNN policy needs n_agents and agent_id
    ...
    def compute(self):
        for agent_id in range(self.num_agents):
            ...
            if self.all_args.algorithm_name == "gnn_happo":
                # GNN requires adjacency matrix and structured observations
                # Reconstruct structured obs from concatenated share_obs
                share_obs_concat = self.buffer[agent_id].share_obs[-1]
